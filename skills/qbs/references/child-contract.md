...
- 子 Skill 在结果之后留一个轻量阅读入口：本次用到的方法、章节/小节位置和继续思考的问题。只有确实用到了才关联，不把阅读变成完成任务的前置打卡。
- 按任务需要添加模板或脚本；不默认每本书必须有脚本。
- `agents/openai.yaml` 的 default_prompt 显式提到 `$<skill-name>`。
- `books/<book-id>.md` 显示官方封面及来源；在线教材没有已核实的正式封面时，可用官网展示图，并明确标注其性质。书籍页还需作者、版本、阅读范围、用途和可复制调用语。封面与书的版权不随项目 MIT 授权。
